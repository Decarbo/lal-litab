		...
		<nav className={`fixed w-full z-[100] transition-all duration-500 border-b ${scrolled ? 'bg-white/85 backdrop-blur-xl shadow-[0_4px_30px_rgba(0,0,0,0.05)] border-gray-100 py-3' : 'bg-transparent border-transparent py-5'}`}>
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div className="flex justify-between items-center h-14">
					{/* Logo Section */}
					<Link href="/" className="group flex-shrink-0 flex items-center gap-2 relative z-50">
					...
					<div className="hidden md:flex items-center space-x-6">
						<Link href="/cart" className="group">
							<button className="text-gray-600 hover:text-[#B22222] relative p-2 transition-colors duration-300 flex items-center justify-center rounded-full hover:bg-red-50">
								<ShoppingCart size={22} className="group-hover:scale-110 transition-transform duration-300" />
								{cart.length > 0 && (
									<span className="absolute 0 top-0 right-0 bg-[#B22222] text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] px-1 flex items-center justify-center shadow-md animate-bounce-short border border-white">
										{cart.length}
									</span>
								)}
							</button>
					...
					<div className="md:hidden flex items-center relative z-50">
                        <Link href="/cart" className="mr-4 relative group">
							<button className="text-gray-600 hover:text-[#B22222] p-1.5 rounded-full hover:bg-red-50 transition-colors">
								<ShoppingCart size={22} />
								{cart.length > 0 && (
									<span className="absolute -top-1 -right-1 bg-[#B22222] text-white text-[10px] font-bold rounded-full min-w-[18px] h-[18px] flex items-center justify-center border border-white">
										{cart.length}
									</span>
								)}
							</button>
						...
						<button
							...
							{isOpen ? <X size={24} className="animate-in spin-in-90 duration-300" /> : <Menu size={24} className="animate-in spin-in-[-90deg] duration-300" />}
						</button>
					</div>
				</div>
			</div>
					...
					<div className="flex flex-col space-y-2">
						{navLinks.map((link, index) => {
                            ...
                            return (
                                ...
                                >
                                    {link.name}
                                    <ChevronRight size={18} className={`transition-transform duration-300 ${isActive ? 'translate-x-1' : 'opacity-50'}`} />
                                </Link>
                            );
                        })}
					</div>

					<div className="mt-auto pt-8 border-t border-gray-100 space-y-4" style={{ 
                        opacity: isOpen ? 1 : 0, 
